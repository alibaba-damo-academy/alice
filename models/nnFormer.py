"""
Mostly copy-paste from nnFormer libarary:
https://github.com/282857341/nnformer
https://github.com/282857341/nnFormer/blob/main/nnformer/network_architecture/nnFormer_synapse.py
"""
...
class WindowAttention_kv(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # print('self.relative_position_bias_table:' ,relative_position_index.shape)
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                input_resolution=(img_size[0] // patch_size[0] // 2 ** i_layer, 
                                  ...
                                  img_size[2] // patch_size[2] // 2 ** i_layer),
        ...
        if self.ape:
            self.absolute_pos_embed = get_sinusoid_encoding_table((img_size[0] // patch_size[0] * img_size[1] // patch_size[1] * img_size[2] // patch_size[2]), embed_dim)
    ...
    def forward(self, x, mask=None):
        ...
        if self.ape:
            x = x + self.absolute_pos_embed.cuda(non_blocking=True)
        ...
        cls_token = self.avgpool(down[-1].flatten(2)).squeeze(2)
        enfeat1 = down[-2]
        return [down, cls_token, enfeat1]
        # return down
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers)[::-1]:
            layer = BasicLayer_up(
                ...
                input_resolution=(
                    img_size[0] // patch_size[0] // 2 ** (len(depths)-i_layer-1), 
                    img_size[1] // patch_size[1] // 2 ** (len(depths)-i_layer-1),
                    img_size[2] // patch_size[2] // 2 ** (len(depths)-i_layer-1)),
                depth=depths[i_layer],
                num_heads=num_heads[i_layer],
                window_size=window_size[i_layer],
                mlp_ratio=mlp_ratio,
...
class nnFormer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.model_down = Encoder(img_size=img_size, window_size=window_size, embed_dim=embed_dim, patch_size=patch_size,
                                            depths=depths, num_heads=num_heads, in_chans=input_channels)
        
        self.decoder = Decoder(img_size=img_size, embed_dim=embed_dim, window_size=window_size[::-1][1:],
    ...
    def forward(self, x_in, mask=None):
        if mask is not None:
            x_in, x_mask = self.mask_model(x_in, mask)
            skips, cls_token, enfeat1 = self.model_down(x_mask)
        else:
            skips, cls_token, enfeat1 = self.model_down(x_in)
        
        seg_outputs=[]
        neck=skips[-1]
        out = self.decoder(neck, skips)
        
        defeat1 = out[0].transpose(1,4)
        ...
        # if mask is not None:
        #     feat_query, feat_anchor  = neck, defeat1
        # else:
        feat_query, feat_anchor = neck, defeat1
        
        #return [cls_query, cls_token2, neck, seg_outputs[-1]]
        return [cls_token, feat_query, feat_anchor, seg_outputs[-1]]
        # return [cls_token1, cls_token2, neck, seg_outputs[-1]]
