...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.D = input_resolution[0]
        self.H = input_resolution[1]
        self.W = input_resolution[2]

        self.attn_mask_dict = {} # {self.H: self.create_attn_mask(self.H, self.W)}


    def create_attn_mask(self, D, H, W):
        # calculate attention mask for SW-MSA
        Dp = int(np.ceil(D / self.window_size)) * self.window_size
        Hp = int(np.ceil(H / self.window_size)) * self.window_size
        Wp = int(np.ceil(W / self.window_size)) * self.window_size

        img_mask = torch.zeros((1, Dp, Hp, Wp, 1))  # 1 Dp Hp Wp 1
