...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=160, patch_size=16, in_chans=1, embed_dim=768):
        super().__init__()
...
class VisionTransformer(nn.Module):
    """ Vision Transformer """
    def __init__(self, img_size=[160], patch_size=16, in_chans=1, num_classes=0, embed_dim=768, depth=12,
    ...
    def interpolate_pos_encoding(self, x, w, h, z):
        npatch = x.shape[1] - 1
        N = self.pos_embed.shape[1] - 1
        # print(x.shape)
        # print(self.pos_embed.shape)
        # print([npatch, N, w, h, z])
        if npatch == N and w == h and w == z:
        ...
        z0 = z // self.patch_embed.patch_size

        w0, h0, z0 = w0 + 0.1, h0 + 0.1, z0 + 0.1
        patch_pos_embed = nn.functional.interpolate(
            patch_pos_embed.reshape(1, int(math.sqrt(N)), int(math.sqrt(N)), dim).permute(0, 3, 1, 2),
            scale_factor=(w0 / math.sqrt(N), h0 / math.sqrt(N), z0 / math.sqrt(N)),
            mode='bicubic',
        )
        assert int(w0) == patch_pos_embed.shape[-3] and int(h0) == patch_pos_embed.shape[-2] and int(z0) == patch_pos_embed.shape[-1]
    ...
    def prepare_tokens(self, x, mask=None):
        ...
        x = x + self.interpolate_pos_encoding(x, w, h, z)
